...
bool command_match_hex_number(string& command, unsigned int& i, uint32_t& num) {
  unsigned int j = i;
...
bool command_match_x(string& command, unsigned int i, bool& data_present, unsigned int& num, uint32_t& data) {
  data_present = false;
...
bool command_match_pc(string& command, unsigned int i, bool& address_present, uint32_t& address) {
...
bool command_match_m(string& command, unsigned int i, bool& data_present, uint32_t& address, uint32_t& data) {
  data_present = false;
...
bool command_match_b(string& command, unsigned int i, bool& address_present, uint32_t& address) {
  address_present = false;
...
bool command_match_csr(string& command, unsigned int i, bool& data_present, uint32_t& address, uint32_t& data) {
  data_present = false;
  if (i == command.length() || command[i] != 'c') return false;
  i++;
...
void interpret_commands(memory* main_memory, processor* cpu, bool verbose) {
  ...
  while (true) {
	...
	data = main_memory->read_word(address);
	cout << setw(8) << setfill('0') << hex << data << endl;
     }
      else {  // Update memory word
        main_memory->write_word(address, data, 0xffffffffUL);
      }
    }
